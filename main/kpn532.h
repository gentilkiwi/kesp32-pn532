...
#define PISR_PN532_ROUTINE gpio_isr_t

class PN532 {

public:
  PN532(const gpio_num_t ss_pin, const gpio_num_t irq_pin, spi_device_handle_t hDevice = NULL);  // Only IRQ
  ~PN532();
...
public:
  static uint8_t InitGlobalSPI(const gpio_num_t clk_pin, const gpio_num_t mosi_pin, const gpio_num_t miso_pin, const int clock_speed_hz = 3000000, const spi_host_device_t spi_host_device = SPI2_HOST);
  static void DeinitGlobalSPI();
  static void PrintHex(const uint8_t *pcbData, const size_t cbData, const uint8_t flags = PN532_PRINTHEX_NONE);
  
private:
  static void ISR_NFC(void *arg);
  
  static spi_host_device_t _spi_host_device;
  static spi_device_handle_t _spi_device;
};
